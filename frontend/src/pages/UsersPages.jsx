...
const teacherColumns = [
    ...
    { header: 'Photo', render: (item) => item.photo ? <img src={`${import.meta.env.VITE_ASSET_URL || 'http://localhost:5001'}${item.photo}`} alt="Teacher" className="w-10 h-10 rounded-full object-cover border border-slate-200" /> : '-' },
    ...
    { header: 'CV', render: (item) => item.cv ? <a href={`${import.meta.env.VITE_ASSET_URL || 'http://localhost:5001'}${item.cv}`} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">View CV</a> : '-' },
];
...
const studentColumns = [
    { header: '#', render: (item, index) => <span className="font-bold text-slate-500">{index + 1}</span> },
    { header: 'Photo', className: 'print:hidden', render: (item) => item.photo ? <img src={`${import.meta.env.VITE_ASSET_URL || 'http://localhost:5001'}${item.photo}`} alt="Profile" className="w-10 h-10 rounded-full object-cover border border-slate-200" /> : '-' },
    { header: 'Name', render: (item) => item.user?.name },
    {
        header: 'Phone/WA', render: (item) => (
...
const graduateColumns = [
    { header: '#', render: (item, index) => <span className="font-bold text-slate-500">{index + 1}</span> },
    { header: 'Photo', render: (item) => item.photo ? <img src={`${import.meta.env.VITE_ASSET_URL || 'http://localhost:5001'}${item.photo}`} alt="Profile" className="w-10 h-10 rounded-full object-cover border border-slate-200" /> : '-' },
    { header: 'Name', render: (item) => item.user?.name },
